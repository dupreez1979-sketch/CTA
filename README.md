...
1. **Supabase**: create a project at supabase.com → **Connect** → copy the **Session pooler** URI (`...pooler.supabase.com:5432`) and substitute your database password. That's `DATABASE_URL`. Migrations run automatically during each Netlify build (`netlify.toml` runs `npm run db:migrate` before `next build`), so there's nothing to apply by hand.
